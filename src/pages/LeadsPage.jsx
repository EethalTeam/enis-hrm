        ...
        <div className="max-h-[60vh] overflow-y-auto space-y-4 pr-4">
          {notes && notes.length > 0 ? (
            ...
            <p className="text-gray-400 text-center py-8">
              No notes history found for this lead.
            </p>
          )}
        </div>
      </DialogContent>
...
const LeadForm = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (payload._id) {
        // UPDATE LEAD
        await apiRequest("Lead/updateLead/", {
          method: "POST",
        ...
        // CREATE LEAD
        await apiRequest("Lead/createLead/", {
          method: "POST",
          body: JSON.stringify(payload),
        });
        toast({
          title: "Lead Added",
          description: `${payload.leadName} has been added.`,
        });
      }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent
        className="glass-effect border-white/10 text-white max-w-4xl"
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={handleChange}
                required
                className="bg-white/5 text-white [&::-webkit-calendar-picker-indicator]:invert"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <Input
              ...
              onChange={handleChange}
              placeholder="e.g., Interested, Not Answering"
              className="bg-white/5"
            />
          </div>
          <div>
          ...
          <div>
            <Label>{lead ? "Add New Note" : "Add Note"}</Label>
            <Input
              name="notes"
      ...
      leads.filter((lead) => {
        ...
        const matchesSearch =
          ...
          (lead.leadPhoneNumber &&
            lead.leadPhoneNumber.toLowerCase().includes(term));

        const matchesStatus =
          statusFilter === "all" || lead.statusId._id === statusFilter;
  ...
  const confirmDelete = async () => {
    try {
      await apiRequest("Lead/deleteLead/", {
        method: "POST",
        body: JSON.stringify({ _id: selectedLead._id }),
      });
      toast({
        title: "Lead Deleted",
        description: `"${selectedLead.leadName}" has been deleted.`,
      });
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      if (
        ...
        (response.leads ||
          response.message ===
            "File received. Leads are being imported in the background.")
      ) {
        toast({
          title: "Import Started",
          description:
            response.message || `${response.leads.length} leads were imported.`,
        });
      ...
      <AnimatePresence>
        {isConfirmOpen && (
          <ConfirmationDialog
            ...
            title={`Delete Lead: ${selectedLead?.leadName}`}
      ...
      <div className="space-y-8">
        ...
        >
          <div>
            ...
            <p className="text-gray-400">Track and manage candidates.</p>
          </div>
