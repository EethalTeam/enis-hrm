...
const AttendanceReportsPage = () => {
  ...
  const summaryStats = useMemo(() => {
    ...
    const totalHours = reportData.reduce(
      (sum, emp) => sum + (emp.totalHoursWorked || 0),
      0,
    );
    const totalWorkingDays = reportData.reduce(
      (sum, emp) => sum + (emp.totalWorkingDays || 0),
      0,
    );
    const totalAvgSum = reportData.reduce(
      (sum, emp) => sum + parseFloat(emp.averageHoursPerDay || 0),
      0,
    );
    const avgHours =
      reportData.length > 0 ? totalAvgSum / reportData.length : 0;
    return {
      totalHours,
      totalWorkingDays,
      avgHours,
  ...
  const handleDownload = () => {
    ...
    link.setAttribute(
      "download",
      `Attendance Report - ${monthName} ${selectedYear}.csv`,
    );
    ...
    <>
      ...
      <div className="space-y-8">
        ...
        >
          <div>
            ...
            <p className="text-gray-400">
              Generate and view monthly attendance summaries.
            </p>
          </div>
          <div className="flex items-center gap-4">
            ...
            >
              <SelectTrigger className="glass-effect w-[180px]">
                <SelectValue />
              </SelectTrigger>
            ...
            >
              <SelectTrigger className="glass-effect w-[120px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent className="glass-effect">
        ...
        >
          ...
          <Card className="metric-card">
            <CardContent className="p-6">
              <p className="text-gray-400">Total Work Hours</p>
              <h3 className="text-3xl font-bold">
          ...
          <Card className="metric-card">
            <CardContent className="p-6">
              <p className="text-gray-400">Avg. Daily Hours</p>
              <h3 className="text-3xl font-bold">
                {formatDuration(summaryStats.avgHours)}
              </h3>
            </CardContent>
          </Card>
        </motion.div>
                        ...
                        reportData.map((emp) => (
                          <tr key={emp.employeeId}>
                            ...
                            <td className="text-green-400">
                              {formatDuration(emp.totalHoursWorked)}
                            </td>
                            <td className="text-yellow-400">
                              {formatDuration(emp.totalBreakHours)}
                            </td>
                            <td className="text-blue-400">
                              {formatDuration(emp.averageHoursPerDay)}
                            </td>
                          </tr>
                        ))
                      ) : (
