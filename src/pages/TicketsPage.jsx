...
const CreateTicketDialog = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await apiRequest("Ticket/createTicket", {
        method: "POST",
  ...
  const filteredProjects =
    user.role === "Client"
      ? projects.filter(
          (project) => String(project.clientId?._id) === String(user._id),
        )
      : projects;
  return (
      ...
      <DialogContent className="glass-effect text-white border-white/10">
        <DialogHeader>
          ...
          <DialogDescription className="text-gray-400">
            Fill in the details to raise a new support ticket.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4 py-4">
          {(user.role === "Super Admin" || user.role === "Client") && (
            <div>
...
const EditTicketDialog = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await apiRequest("Ticket/updateTicket", {
        method: "POST",
        body: JSON.stringify(body),
      });
...
const UpdateStatusDialog = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await apiRequest("Ticket/updateTicketStatus", {
        method: "POST",
        body: JSON.stringify(body),
      });
...
const AssignTicketDialog = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await apiRequest("Ticket/assignTicket", {
        method: "POST",
        body: JSON.stringify(body),
      });
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="glass-effect text-white border-white/10">
        <DialogHeader>
          ...
          <DialogDescription className="text-gray-400">
            Assign ticket{" "}
            <span className="font-bold text-white">{ticket?.ticketId}</span> to
            an employee.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4 py-4">
...
const TicketsPage = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const optionsRes = await apiRequest("Ticket/getTicketOptions", {
          method: "POST",
        });
  ...
  useEffect(() => {
    const fetchInitialDataproject = async () => {
      try {
        const optionsRes = await apiRequest("Project/getAllProjectsforassign", {
          method: "POST",
        });
  ...
  const fetchTickets = useCallback(async () => {
    ...
    try {
      const body = {
        ...
        userId: user._id, // send current user ID
        role: user.role, // send current user role
      };

      const response = await apiRequest("Ticket/getAllTickets", {
        method: "POST",
        body: JSON.stringify(body),
      });
  ...
  }, [debouncedSearchTerm, statusFilter, priorityFilter, user]);
  useEffect(() => {
    fetchTickets();
  }, [fetchTickets]);
  ...
  const fetchStats = useCallback(async () => {
    try {
      // Assuming you have user info from context or state
      const userId = user?._id; // your logged-in user ID
      const role = user?.role; // "Superadmin" or employee role

      const response = await apiRequest("Ticket/getTicketStats", {
        method: "POST",
  ...
  return (
    <>
      ...
      <CreateTicketDialog
        ...
        employees={employees}
        projects={projects}
        onTicketCreated={handleDataUpdate}
      />
      <AssignTicketDialog
        ...
        employees={employees}
        onTicketAssigned={handleDataUpdate}
      />
      <EditTicketDialog
        ...
        ticket={selectedTicket}
        priorities={priorities}
        onTicketUpdated={handleDataUpdate}
      />
      <UpdateStatusDialog
        open={isStatusDialogOpen}
        setOpen={setIsStatusDialogOpen}
        ticket={selectedTicket}
        statuses={statuses}
        onTicketUpdated={handleDataUpdate}
      />
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <div className="flex-1 relative">
                  ...
                  <Input
                    ...
                    className="pl-10 glass-effect border-white/10 text-white"
                  />
                </div>
                <Select value={statusFilter} onValueChange={setStatusFilter}>
                  <SelectTrigger className="glass-effect w-full sm:w-48">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="glass-effect border-white/10 text-white">
                    ...
                    <tbody>
                      {tickets.map((ticket) => (
                        ...
                        >
                          ...
                          <td className="py-4 px-4 text-center">
                            <span
                              className={`px-2 py-1 rounded-full text-xs font-semibold ${getPriorityColor(ticket.priority)}`}
                            >
                              {ticket.priority}
                            </span>
                          </td>
