...
const PayrollPage = () => {
  ...
  const handleProcessPayroll = () => {
    ...
    setTimeout(() => {
      ...
      toast({
        ...
        description:
          "All employee salaries for the selected month have been processed.",
        variant: "default",
  ...
  const generatePayslip = (record) => {
    const doc = new jsPDF();
    const monthYear = new Date(selectedMonth + "-02").toLocaleDateString(
      "en-US",
    ...
    doc.text(
      `Payslip ID: PAY-${record.id.slice(4)}-${selectedMonth.replace("-", "")}`,
      20,
      72,
    );
    ...
    earnings.forEach((earn) => {
      const amount =
        earn.type === "percentage"
          ? baseSalary * (earn.value / 100)
          : earn.value;
    ...
    deductions.forEach((deduct) => {
      const amount =
        deduct.type === "percentage"
          ? record.grossSalary * (deduct.value / 100)
          : deduct.value;
    ...
    doc.save(
      `Payslip-${record.employee.replace(" ", "_")}-${selectedMonth}.pdf`,
    );
  ...
  const handleDownloadReport = () => {
    toast({
      ...
      description:
        "A comprehensive payroll report isn't implemented yet—but individual payslips work! 🚀",
    });
  };
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Comprehensive payroll management system with automated calculations, tax compliance, and detailed reporting capabilities."
        />
        <meta property="og:title" content="Payroll Management - ENIS-HRMS" />
        <meta
          property="og:description"
          content="Streamline payroll processing with automated calculations, compliance management, and comprehensive financial reporting."
        />
      </Helmet>
        ...
        >
          <div>
            ...
            <p className="text-gray-400">
              Manage employee compensation based on defined payroll masters
            </p>
          </div>
          <div className="flex gap-3">
            <Button
              ...
              disabled={
                processing || payrollData.every((p) => p.status === "Paid")
              }
            >
              <Play className="w-4 h-4 mr-2" />
              {processing
                ? "Processing..."
                : payrollData.every((p) => p.status === "Paid")
                  ? "Payroll Paid"
                  : "Process Payroll"}
            </Button>
            <Button
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-400">
                      Select month to view payroll details
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                    ...
                    {payrollData.map((record, index) => (
                      ...
                      >
                        ...
                        <td className="text-white font-semibold">
                          ₹
                          {record.netSalary.toLocaleString(undefined, {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2,
                          })}
                        </td>
