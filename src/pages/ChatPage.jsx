          ...
          <div>
            ...
            <div className="max-h-60 overflow-y-auto space-y-2 rounded-md border border-white/10 p-2 mt-2 scrollbar-thin scrollbar-thumb-slate-700">
              {allEmployees.map((emp) => (
                <div
                  key={emp._id}
                  className="flex items-center justify-between rounded-md p-2 hover:bg-white/10 cursor-pointer"
                ...
                >
                  ...
                  <input
                    ...
                    readOnly
                    className="form-checkbox h-5 w-5 text-blue-500 bg-transparent border-gray-300 rounded focus:ring-blue-500"
                  />
                </div>
              ))}
            </div>
...
const AddMembersDialog = ({
  ...
  const availableEmployees = allEmployees.filter(
    (emp) => !currentGroup?.members.some((member) => member._id === emp._id),
  );
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isConfirming ? "bg-red-900/50" : "hover:bg-white/5",
                )}
              >
...
const ChatPage = () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setActiveGroup((prevActiveGroup) => {
          const updatedActiveGroup = response.groups.find(
            (g) => g._id === prevActiveGroup?._id,
          );
  ...
  const handleCreateGroup = async (groupData) => {
    try {
      ...
      toast({
        title: "Group Created!",
        description: `"${groupData.groupName}" has been created.`,
      });
      fetchUserGroups();
    } catch (error) {
      toast({
  ...
  const handleAddMembers = async (membersData) => {
    try {
      await apiRequest(`Group/AddMembers`, {
        method: "POST",
        body: JSON.stringify(membersData),
      });
      toast({
        title: "Members Added!",
        description: "New members have been added.",
      });
      fetchUserGroups();
    } catch (error) {
      toast({
  ...
  const isCurrentUserAdmin = activeGroup?.admins.some(
      ...
      <AnimatePresence>
        ...
        {isManageMembersOpen && (
          <ManageMembersDialog
            ...
            setOpen={setIsManageMembersOpen}
            group={activeGroup}
            currentUser={user}
              ...
              {groups.map((group) => (
                <Button
                  ...
                  className={cn(
                    ...
                    activeGroup?._id === group._id && "bg-white/10",
                  )}
              ...
              <>
                ...
                <CardContent className="flex-grow p-4 space-y-4 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-700">
                  {messages.map((msg) => {
                    ...
                    const isSenderStillMember = activeGroup.members.some(
