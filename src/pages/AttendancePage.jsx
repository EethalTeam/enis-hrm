...
const calculateTotalBreakHours = (sessions) => {
  if (!sessions || !Array.isArray(sessions)) return 0;
  return sessions.reduce(
    (total, session) => total + (session.totalBreakHours || 0),
    0,
  );
};
...
const processAttendanceRecord = (record) => {
  const totalWorkedHours =
    record.totalWorkedHours ?? calculateTotalHours(record.sessions);
  const totalBreakHours =
    record.totalBreakHours ?? calculateTotalBreakHours(record.sessions);
...
const AttendanceDetailDialog = ({
      ...
      const fetchHolidayAndLeaveData = async () => {
        ...
        try {
          ...
          if (leaveResponse?.success) {
            const approvedLeaves = (leaveResponse.leaves || []).filter(
              (l) => l.RequestStatusId?.StatusName === "Approved",
            );
  ...
  const holidaysMap = useMemo(() => {
    const map = new Map();
    holidays.forEach((h) => {
      const dateString = new Date(h.date).toLocaleDateString("en-IN", {
        timeZone: istTimeZone,
      });
  ...
  const getAttendanceForDate = (day) => {
    const dayDateIST = new Date(
      selectedYear,
      selectedMonth,
      day,
    ).toLocaleDateString("en-IN", { timeZone: istTimeZone });
    return attendanceRecords.find((record) => {
      const recordDateIST = new Date(record.date).toLocaleDateString("en-IN", {
        timeZone: istTimeZone,
      });
  ...
  const getDayBackgroundColor = (day) => {
    ...
    const dayDateString = dayDate.toLocaleDateString("en-IN", {
      timeZone: istTimeZone,
    });
    ...
    <div className="bg-slate-700/50 rounded-lg p-3 space-y-2">
      <div className="flex justify-between items-center">
        ...
        <span className="text-xs text-gray-400">
          {formatDuration(session.workedHours)}
        </span>
      </div>
      ...
      {session.breaks && session.breaks.length > 0 && (
        <div className="space-y-1">
          ...
          {session.breaks.map((b, i) => (
            <div
              key={i}
              className="flex justify-between text-xs bg-slate-600/50 rounded p-2"
            >
              <span className="text-gray-300">
                {formatTime(b.breakStart)} - {formatTime(b.breakEnd)}
              </span>
              <span className="text-yellow-400">
                {formatDuration(b.breakDuration)}
              </span>
            </div>
          ))}
          <div className="text-xs text-right text-yellow-400">
            Total Break: {formatDuration(session.totalBreakHours)}
          </div>
        </div>
      )}
    </div>
      ...
      <DialogContent className="glass-effect border-white/10 text-white max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription className="text-gray-400">
            {employee?.department} • Click on any day to see its details
          </DialogDescription>
        </DialogHeader>
        <div className="flex flex-col md:flex-row gap-6 h-[70vh] md:h-auto overflow-y-auto">
          <div className="flex-1 space-y-4">
            <div className="flex gap-4">
              ...
              >
                <SelectTrigger className="glass-effect border-white/10 w-48">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="glass-effect border-white/10 text-white">
                  {monthNames.map((month, i) => (
                    <SelectItem key={i} value={i} className="hover:bg-white/10">
                      {month}
                    </SelectItem>
                  ))}
                </SelectContent>
              ...
              >
                <SelectTrigger className="glass-effect border-white/10 w-32">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="glass-effect border-white/10 text-white">
            ...
            <div className="grid grid-cols-7 gap-2">
              ...
              {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(
                (day) => {
                  ...
                  const currentDayIST = currentDate.toLocaleDateString(
                    "en-IN",
                    {
                      timeZone: istTimeZone,
                    },
                  ...
                  const isToday = currentDayIST === todayIST;
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`w-full h-8 rounded flex items-center justify-center text-sm font-medium ${getDayBackgroundColor(
                          day,
          ...
          <div className="w-96 space-y-4">
            <AnimatePresence mode="wait">
              {selectedDay ? (
                <motion.div
                  key={`${selectedDay.type}-${selectedDay.day}`}
                    ...
                    <Card className="glass-effect border-white/10">
                      <CardHeader className="pb-3">
                        <CardTitle className="text-lg text-white">
                          {monthNames[selectedMonth]} {selectedDay.day},{" "}
                          {selectedYear}
                        </CardTitle>
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="text-green-400 font-medium">
                            {formatDuration(selectedDay.data.totalWorkedHours)}
                          </span>{" "}
                        </div>
                        <div className="flex justify-between text-sm">
                          {" "}
                          ...
                          <span className="text-yellow-400 font-medium">
                            {formatDuration(selectedDay.data.totalBreakHours)}
                          </span>{" "}
                        </div>
                      </CardHeader>
                      <CardContent className="space-y-3 overflow-y-auto">
                        {selectedDay.data.sessions.map((s, i) => (
                          <SessionLog key={i} session={s} index={i} />
                        ))}
                      </CardContent>
                    </Card>
                  )}
...
const AttendancePage = () => {
    ...
    try {
      ...
      if (response?.success && response.attendance) {
        ...
        response.attendance.forEach((record) => {
          ...
          const id =
            processedRecord.employeeId._id || processedRecord.employeeId;
  ...
  const getEmployeeAttendanceHistory = async (employeeId) => {
    try {
      const response = await apiRequest(
        "Attendance/getEmployeeAttendanceHistory/",
        {
          method: "POST",
          body: JSON.stringify({ employeeId, _id: user._id, role: user.role }),
        },
      );
      return response?.success && response.attendance
        ? response.attendance.map(processAttendanceRecord)
        : [];
    } catch (e) {
  ...
  const getEmployeeAttendanceForDate = (employeeId) => {
    ...
    const selectedDateString = localDate.toLocaleDateString("en-IN", {
      timeZone: istTimeZone,
    });
  ...
  const getStatusText = (attendance) => {
    ...
    if (attendance.sessions.some((s) => s.breaks?.some((b) => !b.breakEnd)))
      return "On Break";
  ...
  return (
    <>
      <Helmet>
        <title>Attendance Daily Log - ENIS-HRMS</title>
      </Helmet>
      <AnimatePresence>
        {isDetailDialogOpen && (
        ...
        >
          <div>
            ...
            <p className="text-gray-400">
              Monitor employee attendance and work sessions.
            </p>
          </div>
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {filteredEmployees.map((employee) => {
                        const attendance = getEmployeeAttendanceForDate(
                          employee._id,
                        );
                        return (
                          <tr
                          ...
                          >
                            ...
                            <td
                              className={`py-4 px-4 font-medium ${getStatusColor(attendance)}`}
                            >
