...
const LateLoginsCard = ({ lateLogins }) => (
  <Card className="glass-effect border-white/10">
    ...
    <CardContent>
      ...
      <Link
        to="/dailyLog"
        className="text-blue-400 hover:text-blue-300 mt-4 block text-sm"
      >
  ...
  <div className="space-y-8">
    ...
    >
      <MetricCard
        ...
        value={stats.todaysLeads.length}
        icon={PhoneForwarded}
        color="blue"
      />
      <MetricCard
        title="Tasks Due Today"
        value={stats.todaysTasks.length}
        icon={CalendarCheck2}
        color="purple"
      />
      <MetricCard
        title="Total Active Leads"
        value={stats.allLeads.length}
        icon={Target}
        color="green"
      />
      <MetricCard
        ...
        value={
          stats.allTasks.filter((val) => val.taskStatusId.name !== "Completed")
            .length
        }
        icon={ClipboardCheck}
        color="orange"
        ...
        <CardContent>
          {stats.todaysLeads.length > 0 ? (
            <ul className="space-y-4">
              {stats.todaysLeads.slice(0, 5).map((lead) => (
                ...
                >
                  ...
                  <span
                    className={`status-badge text-xs bg-gray-500/20 text-gray-300`}
                  >
            ...
            <p className="text-gray-400">
              No lead follow-ups scheduled for today.
            </p>
          )}
          <Link
            to="/allLeads"
              ...
              {stats.todaysTasks.slice(0, 5).map((task) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-gray-400">
                      Assigned to: {task.assignedTo[0]?.name || "N/A"}
                    </p>
                  </div>
...
const EmployeeDashboard = ({ stats }) => (
  ...
  <div className="space-y-8">
    ...
    >
      <Card className="glass-effect border-white/10">
        <CardHeader>
          <CardTitle className="text-white">My Follow-ups for Today</CardTitle>
        </CardHeader>
        <CardContent>
          {stats.todaysLeads.length > 0 ? (
            <ul className="space-y-4">
      ...
      <Card className="glass-effect border-white/10">
        <CardHeader>
          <CardTitle className="text-white">My Tasks Due Today</CardTitle>
        </CardHeader>
        <CardContent>
          {stats.todaysTasks.length > 0 ? (
            <ul className="space-y-4">
...
const DashboardPage = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (isAdmin) {
          ...
          apiCalls.push(
            apiRequest("DashBoard/getTodayPermissions", commonPayload),
          );
  ...
  const todaysLeads = useMemo(() => {
    ...
    return allLeads.filter(
      (lead) => lead.nextFollowUp?.split("T")[0] === today,
    );
  }, [allLeads]);
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Dashboard overview of key metrics, leads, and tasks."
        />
      </Helmet>
