...
const ProjectForm = ({
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => {
      ...
      //  if startDate changed → reset invalid endDate
  ...
  const createProject = async (data) => {
    try {
      ...
      toast({
        title: "Project Added",
        description: `${formData.projectName} has been added to the system.`,
      });
      setOpen(false);
      SetData([]);
      getAllProjects();
        ...
        <DialogHeader>
          ...
          <DialogDescription className="text-gray-400">
            {project
              ...
              : "Provide the details for the new project."}
          </DialogDescription>
        </DialogHeader>
...
const ProjectsPage = () => {
  const { projects, employees, addProject, updateProject, deleteProject } =
    useData();
  ...
  const deleteProjectApi = async (id) => {
    try {
      const response = await apiRequest("Project/deleteProject", {
        method: "POST",
  ...
  const handleSave = (projectData) => {
    if (selectedProject) {
      updateProject({ ...projectData, id: selectedProject._id });
      toast({
        title: "Project Updated",
      ...
      toast({
        ...
        description: `"${projectData.projectName}" has been created.`,
      });
    }
  };
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Manage all company projects from a centralized dashboard."
        />
      </Helmet>
      ...
      {isConfirmOpen && (
        <ConfirmationDialog
          ...
          title={`Delete Project: ${selectedProject.projectName}`}
          description="This will delete the project and all associated tasks. This action is irreversible."
        />
      )}
        ...
        >
          <div>
            ...
            <p className="text-gray-400">
              Oversee all ongoing and completed company projects.
            </p>
          </div>
          {Permissions.isAdd && (
            <Button
              ...
              <Card className="glass-effect h-full flex flex-col">
                <CardHeader>
                  <div className="flex justify-between items-start">
                    ...
                    <DropdownMenu>
                      {(Permissions.isDelete || Permissions.isEdit) && (
                    ...
                    <div>
                      ...
                      <div className="flex -space-x-2">
                        {project.assignedEmployees
                          .map((empId) =>
                            Employees.find((e) => e._id === empId._id),
                          )
                          .filter(Boolean)
                          .slice(0, 5)
                          .map((emp) => (
                        ...
                        {project.teamSize > 5 && (
                          <div className="w-8 h-8 rounded-full bg-slate-600 flex items-center justify-center text-xs font-bold border-2 border-slate-700">
                            +{project.teamSize - 5}
                          </div>
                        )}
                      </div>
