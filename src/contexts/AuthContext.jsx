...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const storedUser =
      localStorage.getItem("hrms_user") || localStorage.getItem("client_user");
    if (storedUser) {
      try {
  ...
  const login = async (email, password) => {
    try {
      ...
      localStorage.setItem(
        "hrms_userEmail",
        JSON.stringify(data.employee.email),
      );
  ...
  const clientlogin = async (email, password) => {
    try {
      ...
      let url = config.Api + "/api/Client/clientlogin/";
      ...
      // connect socket
      socket.connect();
      socket.emit("joinRoom", { clientId: data.client._id });
  ...
  //     let UserName = JSON.parse(localStorage.getItem("hrms_user")).UserName;
  //     const response = await fetch(url, {
  //       method: "POST",
  //       headers: {
  //         "Content-Type": "application/json",
  //       },
  //       body: JSON.stringify({ email: email.email || UserName }),
  //     });
  ...
  const logout = async () => {
    try {
      ...
      const role = storedUser.role || "Client"; // Default to 'Client' if role is missing
      const email = storedUser.email || storedUser.UserName;

      const url = config.Api + "/api/Employee/logout/";
      ...
      // Clear state
      setUser(null);
      localStorage.removeItem("hrms_user"); // cleaner than clear()
      localStorage.removeItem("client_user"); // cleaner than clear()
